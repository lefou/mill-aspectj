...
`def aspectjCompileMode: CompileMode`::
+
--
Configures want kind of sources the `ajc` compiler is processing.

`CompileMode.FullSources` (_default_):: Compile all source files (`+*.java+` and `+*.aj+`).
`CompileMode.OnlyAjSources`::
  Only compile `+*.aj+` files but weave-compile all other classes given via `-inpath`. This setup requires another compiler to compile the Java (and other) source files to `.class` files.
  If you don't mix-in other compiler modules before `AspectJModule`, Zinc as configured by the `JavaModule` will be used.
...
`def ivyDeps: T[Agg[Dep]]`::
  ...
  If you do not use `super.ivyDeps()` when overriding this def, you need to provide the `aspectjrt.jar` manually.
...
object main extends ScalaModule with AspectjModule {
  // Only compile aj-files from source
